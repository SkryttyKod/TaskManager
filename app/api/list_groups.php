...
try {
    ...
    $consultaGrupos = $conexion->query(
        'SELECT
            ...
            COUNT(s.id) AS total_tareas,
            COALESCE(SUM(CASE WHEN s.completada = 1 THEN 1 ELSE 0 END), 0) AS tareas_completadas
        FROM tareas g
        LEFT JOIN subtareas s ON s.id_tarea = g.id
    ...
    $consultaTareas = $conexion->query(
        'SELECT
            ...
            fecha_actualizacion
        FROM subtareas
        ORDER BY id_tarea ASC, posicion ASC, id ASC'
    );
